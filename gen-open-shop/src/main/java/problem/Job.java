...
public class Job {
    ...
    /*
    * Key: time
    * Value: machine (operation) number
    * */
    Map<Integer, Integer> processed;

    public Job(List<Integer> operations) {
    ...
    public int findGap(int startTime, int endTime, int length) {
        ...
        for (int left : processed.keySet()) {
            ...
            if (left >= startTime && left < endTime) {
                Arrays.fill(buf, left - startTime, right > endTime ? endTime - startTime : right - startTime, false);
            }
            if (right > startTime && right <= endTime) {
                Arrays.fill(buf, left < startTime ? 0 : left - startTime, right - startTime, false);
            }
        }
        int p = -1;
        int sum = 0;
    ...
    public int findGap(int startTime, int length) {
        ...
        for (int time : processed.keySet()) {
            if (time < startTime && time + operations.get(processed.get(time)) > startTime) {
